...
namespace ElectroStore1.Pages.MainPages
{
    public partial class SearchPage : Page
    {
        ...
        public SearchPage(string searchQuery, int userId, int roleId)
        {
            InitializeComponent();
        ...
        private async Task LoadProductsAsync()
        {
            try
            {
                ...
                await Dispatcher.InvokeAsync(() =>
                {
                    ...
                    SearchResultsControl.ItemsSource = Products;

                    UpdatePaginationButtons();
                    UpdateResultsText();

                    LoadingIndicator.Visibility = Visibility.Collapsed;
                    NoResultsText.Visibility = products.Count == 0 ? Visibility.Visible : Visibility.Collapsed;
                });
            }
            catch (Exception ex)
            {
                Logger.Error("Ошибка при загрузке товаров поиска", ex);
                ...
                await Dispatcher.InvokeAsync(() =>
                {
                    ...
                    NoResultsText.Text = "Ошибка загрузки результатов";
                });
            }
        }
            ...
            await Task.Run(() =>
            {
                using (var connection = DBConnection.GetConnection())
                {
                    ...
                    string countQuery = @"
                        ...
                        WHERE p.IsActive = 1 
                        AND (p.ProductName LIKE @SearchQuery OR b.BrandName LIKE @SearchQuery)";
        ...
        private void UpdateResultsText()
        {
            ...
            {
                SearchResultsText.Text = $"Все товары ({_totalItems})";
            }
            else
            {
                SearchResultsText.Text = $"Найдено {_totalItems} товаров по запросу \"{_searchQuery}\"";
            }
        }
